package Oubliette;
/**
 * @author Kevin Rapa
 */
import Tunnels.Dungeon_Tunnel;

public class Ou62 extends Dungeon_Tunnel {
// ============================================================================    
    public Ou62(String name, String ID) {
        super(name, ID);
        this.description= "You are at the bottom of a square pit, about 8 feet \n" +
                          "across. You can see a hanging bowl of fire high above, \n" +
                          "however barely any light reaches this area. The walls \n" +
                          "and floors are cold stone brick, and it feels as \n" +
                          "though the floor is covered in straw. A single large \n" +
                          "spike protrudes out the center of the floor.";
    }
// ============================================================================
}