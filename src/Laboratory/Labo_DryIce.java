...
public class Labo_DryIce extends Furniture implements Container {
    ...
    public Labo_DryIce () {
        ...
        this.inv = new Ice_Inventory(DRY_ICE, DRY_ICE, DRY_ICE, DRY_ICE);
    ...
    @Override public String getSearchDialog() {
        if (this.searchable)
            ...
            return "The bromine is chilling. Better give it more time.";
    }
    // ========================================================================    
    // ************************************************************************
    // ========================================================================   
    private class Chill extends Thread {
        ...
        @Override public void run() {
            ...
            try {
                synchronized (this) {
                    ...
                    this.wait(30000);
                    GUI.out("The bromine is probably cold enough now. Better go check it.");
                    this.reOpen();
                }
        ...
        public void reOpen() {
            for (Item i : Labo_DryIce.this.getInv())
                if (i.toString().matches("bromine, \\d{1,2}mL"))
                    Labo_DryIce.this.getInv().remove(i);
            
            Labo_DryIce.this.getInv().add(new Item("chilled " + volume, "the bromine has condensed to a rust-colored liquid."));
    ...
    private class Ice_Inventory extends Inventory {
        ...
        @Override public boolean add(Item item) {
            if (! busy && item.toString().matches("bromine, \\d{1,2}mL")) {
